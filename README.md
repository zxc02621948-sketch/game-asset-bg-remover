...
# Game Asset BG Remover · 遊戲素材去背助手
...
### Demo 連結
GitHub Pages Demo：https://zxc02621948-sketch.github.io/game-asset-bg-remover/
...
- 強化常見灰白棋盤格殘留清除
- 可依圖片類型選擇處理方式:混合素材 / 煙霧光效 / 實體物件
- 提供常用預設:黑底物件 / 煙霧光效 / 棋盤背景 / 玻璃透鏡 / 綠幕素材
- 可即時預覽:去背結果 / 黑白範圍 / 原圖
- 可切換預覽底色:棋盤 / 遊戲深色 / 黑色 / 白色
- 支援常用微調:清除強度 / 保留細節 / 邊緣柔和 / 智能修邊 / 修邊強度
- 支援點選補修:點一下清殘留 / 標記玻璃煙霧區 / 標記不要動的區域
- 支援只找相連區域或找整張相近顏色
- 支援半透明區自動清小黑點
- 支援綠幕去背與去綠 / 青色邊
- 可匯出目前圖片為透明 PNG、批量匯出 ZIP
- 顯示批次效能風險提示(張數 / 最大解析度 / 總像素 / 預估記憶體)
...
### 與動畫格對齊工具連動
本工具可以和 [AI 動畫圖格對齊工具](https://zxc02621948-sketch.github.io/ai-sprite-align-tool/) 搭配使用。建議流程:去除黑底/白底/綠幕背景 → 按「送到動畫格對齊」→ 設定欄列數並對齊每一格 → 匯出整理後的透明 spritesheet。按下「送到動畫格對齊」會先把目前圖片暫存在本機瀏覽器,再開啟動畫格對齊工具並自動載入。
...
### 免責說明
本工具是遊戲素材前處理與去背輔助工具,不保證能在所有圖片上產生完美結果。去背品質會受原圖條件影響(背景與主體顏色太接近、原圖已有壓縮雜訊、透明材質與背景混在一起、特效邊緣過於細碎、光效煙霧玻璃與實體混在同一張)。所有處理都在本機瀏覽器進行、不會主動上傳圖片;但使用者仍應自行確認素材授權、來源、商業使用權與最終輸出結果。

### 目前限制
- 第一版偏向瀏覽器本地處理,不適合一次匯入大量超高解析圖片
- 對複雜半透明材質仍可能需要手動點選補修
- 棋盤格背景若已被烘進圖片中,工具只能估算;灰色主體或低飽和素材仍可能需手動標記保護區
- 煙霧 / 光效模式預設較保守,會優先避免把實體物件整體打薄
- 綠幕素材如果本體也有大量綠 / 青色,可能需要降低去綠邊或加保護區
- 工具目前不使用 AI 模型,主要靠影像運算、顏色判斷與使用者點選輔助

### 更新日誌
#### 2026-06-28
- 預覽支援滾輪縮放,並新增放大 / 縮小 / 重置按鈕與縮放比例顯示
- 新增 Ctrl + Z 快捷鍵,可快速撤銷上一筆點選補修

### 未來規劃
- 增加筆刷式保護 / 清除工具、選區預覽顏色、一鍵清除小色邊
- 增加黑白雙底 Alpha 還原模式、更好的玻璃 / 煙霧材質處理
- 增加批量設定套用與單張覆蓋設定、匯出 WebP、輸出尺寸與命名規則
- 增加低記憶體批量處理模式、操作教學與範例素材
